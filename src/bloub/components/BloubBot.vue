...
import { lookTarget, TURN_TIME, type GazeScript } from '@/bloub/ui/gaze'
import {
  DEFAULT_EXPRESSION,
  EXPRESSION_BY_ID
...
import { blockAt, defaultCycle, offsetOf, type Block } from '@/bloub/cycles'
...
const props = withDefaults(
  defineProps<{
    ...
    color?: string
    /** Optional product color override; geometry and timing remain upstream-accurate. */
    colorHex?: string
    /** identifiant d'expression de repos du personnalisateur */
    expression?: string
    /** couleur du fond, utilisee pour la brume de profondeur des particules */
    paper?: string
    /** Backing color revealed through the measured eye cut-outs. */
    eyeColor?: string
    /**
...
const ink = computed(() => props.colorHex ?? COLOR_BY_ID.get(props.color)?.hex ?? '#0a0a0c')
...
function tick(ms: number) {
  ...
  // A full measured SVG sample is expensive on the Pi. Idle motion is slow by
  // design, so sample it sparsely unless a mouse gaze is active; processing
  // states retain a higher rate. The microphone halo and speech envelope are
  // separate lightweight animations and remain responsive.
  const frameInterval = state.value === 'idle' && pointer === null ? 160 : 40
  if (renderedAt && ms - renderedAt < frameInterval) return
  renderedAt = ms
...
function startAnimation() {
  if (raf || props.frozenAt !== undefined || document.hidden) return
  last = 0
  renderedAt = 0
  raf = requestAnimationFrame(tick)
}
...
function updateAnimationActivity() {
  if (!mounted) return
  if (props.frozenAt !== undefined || document.hidden) stopAnimation()
  else startAnimation()
}
...
onMounted(() => {
  mounted = true
  document.addEventListener('visibilitychange', updateAnimationActivity)
...
onBeforeUnmount(() => {
  mounted = false
  stopAnimation()
  document.removeEventListener('visibilitychange', updateAnimationActivity)
  detach()
})
...
<template>
  ...
  >
    ...
    <g :opacity="frame.bodyAlpha">
      ...
      -->
      <path :d="frame.bodyPath" :fill="props.eyeColor" />
      <g :mask="`url(#${maskId})`">
    ...
    <!--
      Hats: optional slot rendered inside the avatar's coordinate space so a
      hat can track the live body geometry (local addition, not upstream).
    -->
    <slot name="hat" :body-path="frame.bodyPath" :body-alpha="frame.bodyAlpha" />
  </svg>
</template>
